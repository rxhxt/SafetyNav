...
from app.utils.auth_utils import get_token, get_onParking, get_offParking
...
@app.route('/getOffParking', methods=['GET'])
def display_offParking():
    # This makes the call to the get_token function in the auth_utils.py file
    response, status_code = get_offParking()
    # If the request is successful, return the token
    if status_code == 200:
        api_token = response
        return jsonify({'message': api_token})
    #If the request fails, return the error message
        ...
        df = pd.read_json('message', orient='index')
        print(df)
        return jsonify({'message': response})
        
    
@app.route('/getOnParking', methods=['GET'])
def display_onParking():
    # This makes the call to the get_token function in the auth_utils.py file
    response, status_code = get_onParking()
    # If the request is successful, return the token
    if status_code == 200:
        api_token = response
        return jsonify({'message': api_token})
